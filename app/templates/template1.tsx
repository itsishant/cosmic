...
export default function Template1({ form, personalInfo, skills, experience, projects }: Props) {
  ...
  <button
    onClick={onClick}
    className={`inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium transition-colors duration-200 ${className}`}
  >
    {children}
  </button>
)
      ...
      <header className="sticky top-0 z-10 border-b border-neutral-800 bg-neutral-900/70 backdrop-blur supports-[backdrop-filter]:bg-neutral-900/60 transition-colors">
        <div className="mx-auto max-w-5xl px-6 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm">
            <a
              href="#skills"
              className="rounded text-white/80 hover:text-white transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-900"
            >
              Skills
            </a>
            <a
...
/>
              ...
              <div
                className="absolute -bottom-2 -right-2 h-5 w-5 rounded-full bg-cyan-400 ring-2 ring-neutral-900 motion-safe:animate-pulse"
                aria-hidden
              />
            </div>
          </div>
          ...
          <ul className="mt-4 flex flex-wrap gap-2">
            {(skills || []).length > 0 &&
              skills.map((skill, idx) => (
                <li
                  key={idx}
                  className="group inline-flex items-center gap-2 rounded-full border border-neutral-800 bg-neutral-800/50 px-3 py-1.5 text-sm text-white/90 transition-all duration-200 hover:-translate-y-0.5 motion-reduce:hover:translate-y-0 hover:border-cyan-400 hover:bg-neutral-800"
                >
          ...
          <div className="mt-4 grid gap-6 md:grid-cols-2">
            {(experience || []).length > 0 &&
              experience.map((exp) => (
                <article
                  key={exp.id}
                  className="relative rounded-xl border border-neutral-800 bg-neutral-900/60 p-5 transition-all duration-300 hover:-translate-y-1 motion-reduce:hover:translate-y-0 hover:shadow-[0_0_0_1px_#22d3ee] hover:border-cyan-400"
                >
                  ...
                  <p className="pl-5 text-sm text-white/70">
                    {exp.start} → {exp.end}
                  </p>
          ...
          <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {(projects || []).length > 0 &&
              projects.map((p) => (
                <article
                  key={p.id}
                  className="group rounded-xl border border-neutral-800 bg-neutral-900/60 p-5 transition-all duration-300 hover:-translate-y-1 motion-reduce:hover:translate-y-0 hover:shadow-[0_0_0_1px_#22d3ee] hover:border-cyan-400"
                >
                  ...
                  {Array.isArray(p?.tags) && p.tags.length > 0 && (
                    <ul className="mt-3 flex flex-wrap gap-2">
                      {p.tags.map((t: string, i: number) => (
                        <li
                          key={i}
                          className="rounded-full border border-neutral-800 bg-neutral-800/50 px-2.5 py-0.5 text-xs text-white/80"
                        >
                          {t}
                        </li>
                      ))}
                  ...
                  {p?.link && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="mt-4 inline-flex text-sm font-medium text-cyan-400 hover:underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-neutral-900 rounded"
                    >
                      Visit project →
                    </a>
                  )}
